...
hardware_interface::CallbackReturn OpenArmHW::on_init(
  ...
  prefix_ = (it == info.hardware_parameters.end()) ? "" : it->second;
  ...
  if (it == info.hardware_parameters.end()) {
    ...
    disable_torque_ = (v == "true" || v == "1" || v == "yes" || v == "on");
  }


  bool can_fd = true;
  auto it_canfd = info.hardware_parameters.find("can_fd");
  if (it_canfd != info.hardware_parameters.end()) {
    std::string v = it_canfd->second;
    std::transform(v.begin(), v.end(), v.begin(), ::tolower);
    can_fd = (v == "true" || v == "1" || v == "yes");
  }

  canbus_ = std::make_unique<CANBus>(
    info.hardware_parameters.at("can_device"),
    can_fd ? CAN_MODE_FD : CAN_MODE_CLASSIC);

  motor_control_ = std::make_unique<MotorControl>(*canbus_);
  if (!motor_control_) {
    RCLCPP_ERROR(rclcpp::get_logger("OpenArmHW"), "motor_control_ allocation failed");
    return CallbackReturn::ERROR;
  }
  // if (USING_GRIPPER) {
  ...
  if (motor_types.size() != curr_dof ||
      can_device_ids.size() != curr_dof ||
      can_master_ids.size() != curr_dof) {
    RCLCPP_ERROR(rclcpp::get_logger("OpenArmHW"),
      "Config size mismatch (types:%zu dev:%zu master:%zu) vs curr_dof:%zu",
      motor_types.size(), can_device_ids.size(), can_master_ids.size(), curr_dof);
    return CallbackReturn::ERROR;
  }
...
hardware_interface::CallbackReturn OpenArmHW::on_activate(
  ...
  // 초기 정렬 루프에 타임아웃(예: 최대 500ms, 10ms × 50회)
  bool zeroed = false;
  const int max_iters = 50;
  for (int iter = 0; iter < max_iters && !zeroed; ++iter) {
    bool all_zero = true;
  ...
  for (size_t i = 0; i < curr_dof; ++i) {
    ...
    if (std::abs(pos_commands_[i] - pos_states_[i]) > POS_JUMP_TOLERANCE_RAD) {
      RCLCPP_WARN(rclcpp::get_logger("OpenArmHW"),
                  "Position jump on joint %s: state %.6f -> cmd %.6f (tolerance %.6f)",
                  info_.joints[i].name.c_str(), pos_states_[i], pos_commands_[i], POS_JUMP_TOLERANCE_RAD);
      // 필요하면 여기서 pos_commands_[i]를 클램프/스무딩
    }
